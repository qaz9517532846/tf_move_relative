...
void tf_MoveRelative::execute(const move_base_msgs::MoveBaseGoalConstPtr &goal)
{
    ...
    try
    {
      ...
      if(linear_.acceleration == 0 || theta_.acceleration == 0)
      {
    ...
    while(ros::ok())
    {
      ...
      double rotationTostart_ = ::atan2(diff_y, diff_x);
      double dist = getDistance(diff_x, diff_y);

      if(fabs(rotationTostart_) > angular_moving_tolerance_ && diff_x * diff_x + diff_y * diff_y > linear_tolerance_sq_)
      {
        pub_vel.linear.x = 0;
        pub_vel.linear.y = 0;
        pub_vel.angular.z = cal_vel(rotationTostart_, theta_);
      }
      else if(fabs(rotationTostart_) <= angular_moving_tolerance_ && diff_x * diff_x + diff_y * diff_y > linear_tolerance_sq_)
      {
        pub_vel.linear.x = cal_vel(dist, linear_);
        pub_vel.linear.y = 0;
        pub_vel.angular.z = cal_vel(rotationTostart_, theta_);
      }
      else if(diff_x * diff_x + diff_y * diff_y < linear_tolerance_sq_)
      {
        pub_vel.linear.x = 0;
        pub_vel.linear.y = 0;
...
double tf_MoveRelative::getDistance(double delta_x, double delta_y)
{
